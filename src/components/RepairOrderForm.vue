<template>
	<v-dialog v-model="showDialog" persistent full-width origin="top center">
		<v-toolbar dark color="primary">
			<v-toolbar-title class="white--text">Repair Order #{{ orderNumber }}</v-toolbar-title>
			<v-spacer></v-spacer>
			<v-text-field
				flat
				solo-inverted
		...
		<v-card>
			...
			<v-card-actions>
				<v-spacer></v-spacer>
				<v-btn large color="success" @click="saveRepairOrder">Save</v-btn>
				<v-btn large color="info" @click="newSale">Sale</v-btn>
				<v-btn large color="error" @click="cancelRepairOrder">Cancel</v-btn>
				<v-spacer></v-spacer>
			</v-card-actions>
		</v-card>
	</v-dialog>
...
export default {
	...
	computed: {
		...mapState(["error"]),
		...mapState("repairOrders", ["orderNumber", "showDialog", "repairOrderID"]),
	...
	methods: {
		...
		resetForm() {
			...
			this.$store.dispatch("repairOrders/init"); // re-init latest repairOrders.
		...
		cancelRepairOrder() {
			this.resetForm();
			this.$store.commit("repairOrders/SET_SHOW_DIALOG", false);
		},
		async newSale() {
			if ((await this.createRepairOrder()) == true) {
				...
				this.$store.commit("sales/SET_REPAIR_ORDER_ID", this.repairOrderID);
				this.$store.commit("sales/SET_SHOW_DIALOG", true);
				this.$store.dispatch("sales/setIsRepairOrder", true);
			} else {
				console.log("wat");
			}
		},
		...
		async createRepairOrder() {
			...
			const repairOrder = await RepairOrder.post(data);

			if (repairOrder.status == 201 || repairOrder.status == 200) {
				return true;
			} else {
				console.log(repairOrder.status);
				console.log(repairOrder.error[0]);
				return false;
			}
		}
	},
